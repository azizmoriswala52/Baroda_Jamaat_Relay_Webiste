...
const ResponseTable = ({
  ...
  if (announcement.responseType === 'APPROVAL' && !selectedApproval) {
    ...
    statsRow = (
      <div className="flex flex-wrap gap-x-8 gap-y-4 px-6 py-4 bg-slate-50 border-b border-slate-200">
        <div className="flex flex-col">
          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Requests</span>
          <span className="text-xl font-bold text-slate-800">{totalResponses}</span>
        </div>
        <div className="flex flex-col">
    ...
    statsRow = (
      <div className="flex flex-wrap gap-x-8 gap-y-4 px-6 py-4 bg-slate-50 border-b border-slate-200 items-center">
        <div className="flex flex-col">
          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Responses</span>
          <span className="text-xl font-bold text-slate-800">{totalResponses}</span>
        </div>
        {Object.entries(rsvpCounts).map(([option, count]: [string, any]) => (
          <div key={option} className="flex flex-col">
            <span className="text-xs font-semibold text-brand-accent uppercase tracking-wider">{option}</span>
        ...
        <div className="flex space-x-2">
          {selectedApproval ? (
            <button
              ...
              className="btn-secondary text-sm font-medium flex items-center px-3 py-1.5 bg-white border-slate-300 shadow-sm hover:bg-slate-50"
            >
              &larr; Back to List
            </button>
          ) : (
            ...
            {selectedApproval ? (
              ...
              >
                <div className="p-6 sm:p-10">
                  <h3 className="text-2xl font-bold text-slate-800 mb-8">{selectedApproval.userId.fullName}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 bg-slate-50 p-6 rounded-xl border border-slate-200">
                    <div>
                      ...
                      <span className="text-sm font-medium text-brand-accent font-mono">{selectedApproval.userId.itsId}</span>
                    </div>
                    <div>
                      <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Mobile</span>
                      <span className="text-sm font-medium text-slate-800">{selectedApproval.userId.mobile}</span>
                    </div>
                    <div>
                      <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Jamaat</span>
                      <span className="text-sm font-medium text-slate-800">{selectedApproval.userId.jamaatName}</span>
                    </div>
                    <div>
                      <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Mohallah</span>
                      <span className="text-sm font-medium text-slate-800">{getMohallaString(selectedApproval.userId.mohalla || 'Burhani')}</span>
                    </div>
                  </div>

                  <div className="mb-8">
                    <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-2">Request Reason / Response</span>
                    <div className="bg-slate-50 p-6 rounded-xl text-sm text-slate-700 whitespace-pre-wrap border border-slate-200 leading-relaxed">
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {responses.map((r: any) => (
                      <tr key={r._id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-4 font-mono text-slate-800">{r.userId.itsId}</td>
                        <td className="px-6 py-4 font-medium text-slate-800">{r.userId.fullName}</td>
                        ...
                        <td className="px-6 py-4">
                          <span className="px-2.5 py-1 bg-white border border-slate-300 text-slate-700 rounded-md text-xs font-bold shadow-sm max-w-[200px] inline-block truncate" title={r.response}>
                            {r.response}
                          </span>
                        </td>
              ...
              <motion.div key="approval-table" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="overflow-x-auto w-full">
                <table className="w-full text-left text-sm text-slate-600 relative">
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {responses.map((r: any) => (
                      <tr key={r._id} onClick={() => setSelectedApprovalId(r._id)} className="hover:bg-slate-50 transition-colors cursor-pointer group">
                        <td className="px-6 py-4 font-mono text-slate-800 group-hover:text-brand-accent">{r.userId.itsId}</td>
                        <td className="px-6 py-4 font-medium text-slate-800">{r.userId.fullName}</td>
                        <td className="px-6 py-4">{r.userId.jamaatName}</td>
                        <td className="px-6 py-4">{getMohallaString(r.userId.mohalla || 'Burhani')}</td>
                        <td className="px-6 py-4">{r.userId.mobile}</td>
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="flex justify-between items-start w-full md:w-auto">
          <div>
            <h2 className="text-2xl font-semibold tracking-tight mb-2 text-brand-accent">Announcements Management</h2>
            <p className="text-slate-500 text-sm">Create announcements and track user RSVP responses.</p>
          </div>
          <button
            onClick={() => refetch()}
            disabled={isFetching}
            className="md:hidden btn-secondary flex items-center shadow-sm overflow-hidden min-h-[38px] px-3 border-slate-300 bg-white hover:bg-slate-50 shrink-0 ml-4 disabled:opacity-50"
            title="Refresh Announcements"
          >
            <RefreshCw className={`w-4 h-4 text-slate-600 ${isFetching ? 'animate-spin text-brand-accent' : ''}`} />
          </button>
        </div>
          ...
          >
            <form onSubmit={handleSubmit} className="clean-panel p-8 relative !overflow-visible">
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wide">Announcement Title <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wide">Content <span className="text-red-500">*</span></label>
                  <textarea
                ...
                <div className="space-y-4 max-w-2xl mt-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className={`block text-xs font-semibold mb-2 uppercase tracking-wide ${formData.targetParentMohallas.includes('All') ? 'text-slate-400' : 'text-slate-500'}`}>Allowed Child Mohallas</label>
                      <MultiSelectDropdown
                ...
                {formData.responseType === 'RSVP' && (
                  <motion.div
                    ...
                    className="max-w-2xl"
                  >
                    <label className="block text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wide">Custom RSVP Options <span className="text-red-500">*</span></label>
                    <input
                      type="text"
      ...
      <div className="clean-panel flex flex-col">
        <div className="bg-slate-200 px-6 py-4 border-b border-slate-300 flex justify-between items-center">
          <h3 className="text-sm font-semibold text-slate-700">Announcement History</h3>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center text-slate-500 w-full">No announcements created yet.</div>
        ) : (
            ...
            <table className="w-full text-left text-sm text-slate-600 relative">
              <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500 font-semibold border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-right whitespace-nowrap border-l border-slate-200">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 bg-white">
                {announcements.map((announcement: any) => (
                  <tr
                    ...
                    className={`group transition-colors ${selectedAnnouncement?._id === announcement._id ? 'bg-sky-50' : 'hover:bg-slate-50'}`}
                  >
                    <td className="px-6 py-4 whitespace-nowrap text-slate-500">
                      {new Date(announcement.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </td>
                    <td className="px-6 py-4 max-w-[200px]">
                      <div className="text-slate-900 font-semibold truncate" title={announcement.title}>{announcement.title}</div>
                      <div className="text-slate-500 text-xs mt-1 truncate" title={announcement.content}>{announcement.content}</div>
                    </td>
                    <td className="px-6 py-4 max-w-[200px]">
                      {announcement.targetParentMohallas && announcement.targetParentMohallas.length > 0 && !announcement.targetParentMohallas.includes('All') ? (
                        <div className="text-sm text-slate-700 truncate" title={announcement.targetParentMohallas.join(', ')}>
                    ...
                    <td className="px-6 py-4 max-w-[200px]">
                      {announcement.targetChildMohallas && announcement.targetChildMohallas.length > 0 && !announcement.targetChildMohallas.includes('All') ? (
                        <div className="text-sm text-slate-700 truncate" title={announcement.targetChildMohallas.join(', ')}>
                    ...
                    <td className="px-6 py-4 text-right space-x-3 whitespace-nowrap border-l border-slate-200">
                      <div className="flex justify-end gap-2">
                        {announcement.responseType !== 'NONE' && (
                          <button
                            onClick={() => setSelectedAnnouncement(announcement)}
                            className={`inline-flex items-center px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${selectedAnnouncement?._id === announcement._id ? 'bg-brand-accent text-white shadow-md' : 'bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 shadow-sm'}`}
                          >
                        ...
                        <button
                          ...
                          className="p-1.5 text-slate-400 hover:text-brand-accent hover:bg-sky-50 rounded transition-colors inline-flex align-middle"
                          title="Edit"
                        >
