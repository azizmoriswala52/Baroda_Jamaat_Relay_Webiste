    ...
    <>
      ...
      <div className="space-y-6 max-w-4xl mx-auto">
        {isLoading || (isFetching && (!announcements || announcements.length === 0)) ? (
          ...
          <div className="flex flex-col items-center justify-center p-16 text-center text-slate-500 min-h-[40vh]">
          ...
          <AnimatePresence>
            {announcements.map((announcement) => {
              ...
              return (
                ...
                >
                  {/* DATE COLUMN */}
                  <div className="bg-slate-50 md:w-48 p-6 md:p-8 flex flex-row md:flex-col justify-between md:justify-center items-center border-b md:border-b-0 md:border-r border-slate-100 shrink-0 text-center">
                    <div className="flex flex-row md:flex-col items-baseline md:items-center space-x-2 md:space-x-0">
                      <span className="text-slate-800 font-bold text-3xl md:text-5xl leading-none md:mb-2">
                    ...
                    {announcement.responseType !== 'NONE' && (
                      <div className="md:mt-6">
                        <span className={`inline-flex items-center justify-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider whitespace-nowrap ${announcement.responseType === 'APPROVAL' ? 'bg-amber-100 text-amber-700' : 'bg-brand-accent/10 text-brand-accent'
                          }`}>
                    ...
                    {announcement.responseType !== 'NONE' && (
                      <div className="mt-6 pt-6 border-t border-slate-100">
                        {announcement.deadline && (
                          <div className="mb-4 flex items-center text-sm font-medium text-slate-500 bg-slate-50 p-3 rounded-lg border border-slate-100">
                            <Clock className="w-4 h-4 mr-2 shrink-0 text-slate-400" />
                            <span>Responses accepted until: <span className="text-slate-700 font-semibold">{new Date(announcement.deadline).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</span></span>
                          </div>
                        )}
                        {activeFormId === announcement._id ? (
                            ...
                            >
                              <h4 className="text-sm font-bold text-slate-800 mb-4 border-b border-slate-100 pb-3 flex items-center">
                                {announcement.responseType === 'APPROVAL' ? (
                                  <><ShieldAlert className="w-4 h-4 mr-2" style={{ color: 'var(--color-brand-accent)' }} /> Request Approval</>
                                ) : (
                                  <><CheckCircle2 className="w-4 h-4 mr-2" style={{ color: 'var(--color-brand-accent)' }} /> Confirm RSVP</>
                                )}
                              </h4>

                              <div className="space-y-4">
                                <div>
                                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">ITS ID</label>
                                  <input
                                    type="text"
                                    readOnly
                                    value={user?.itsId || ''}
                                    className="input-field bg-slate-100 cursor-not-allowed font-mono text-slate-600"
                                  />
                                </div>

                                {announcement.responseType === 'RSVP' && announcement.rsvpOptions && announcement.rsvpOptions.length > 0 && (
                                  <div>
                                    <label className="block text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wide">Will Attend <span className="text-red-500">*</span></label>
                                    <div className="space-y-2">
                                      {announcement.rsvpOptions.map((opt: string, idx: number) => (
                                        <label key={idx} className={`flex items-start p-3 border rounded-lg cursor-pointer transition-colors ${selectedRsvpOption === opt ? 'border-brand-accent bg-brand-accent/5' : 'border-slate-200 hover:border-slate-300'}`}>
                                          <div className="flex items-center h-5">
                                            <input
                                              ...
                                              className="w-4 h-4 text-brand-accent accent-brand-accent border-slate-300 focus:ring-brand-accent"
                                            />
                                          </div>
                                ...
                                {announcement.responseType === 'APPROVAL' && (
                                  <div>
                                    <label className="block text-xs font-semibold text-slate-500 mb-1.5">Reason for request <span className="text-red-500">*</span></label>
                                    <textarea
                                      value={reason}
                          ...
                          <div className="clean-panel p-5 bg-slate-50 border border-slate-200">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 border-b border-slate-200 pb-3">
                            ...
                            <div className="space-y-4 opacity-80 pointer-events-none">
                              <div>
                                ...
                                <input
                                  ...
                                  readOnly
                                  value={user?.itsId || ''}
                                  className="input-field bg-slate-100 font-mono text-slate-600"
                                />
                              </div>

                              {announcement.responseType === 'RSVP' && announcement.rsvpOptions && announcement.rsvpOptions.length > 0 && (
                                <div>
                                  <label className="block text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wide">Selected RSVP Option</label>
                                  <div className="space-y-2">
                                    {announcement.rsvpOptions.map((opt: string, idx: number) => (
                                      <label key={idx} className={`flex items-start p-3 border rounded-lg ${announcement.userResponse === opt ? 'border-brand-accent bg-brand-accent/5' : 'border-slate-200 bg-white'}`}>
                                        <div className="flex items-center h-5">
                                          <input
                                            type="radio"
                                            readOnly
                              ...
                              {announcement.responseType === 'RSVP' && (!announcement.rsvpOptions || announcement.rsvpOptions.length === 0) && (
                                <div>
                                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">RSVP Status</label>
                                  <input
                                    type="text"
                                    readOnly
                              ...
                              {announcement.responseType === 'APPROVAL' && (
                                <div>
                                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Reason for request</label>
                                  <textarea
                                    value={announcement.userResponse}
                                    readOnly
                                    className="input-field min-h-[80px] resize-none bg-slate-100 text-slate-600"
                                  />
                                </div>
                              )}
                            </div>
                            ...
                            {((announcement.responseType === 'RSVP' && announcement.submissionCount >= 3) ||
                              ...
                              <div className="flex flex-col sm:flex-row sm:items-center justify-between">
                                <div className="mb-4 sm:mb-0">
                                  ...
                                  <p className="text-xs text-slate-500 mt-1">Please submit your response to proceed.</p>
                                </div>
                                <button
                                  onClick={() => {
                                    setActiveFormId(announcement._id);
